...
public class ObjectExplorer {
        ...
        while (!stack.isEmpty()) {
            ...
            if (valueClass.isArray()) {
                ...
                for (int i = Array.getLength(value) - 1; i >= 0; i--) {
                    ...
                    if (childValue == null) {
                        if (features.contains(Feature.VISIT_NULL)) visitor.visit(chain.appendArrayIndex(i, childValue));
                        continue;
                    }
                ...
                for (Field field : getAllFields(value)) {
                    ...
                    if (childValue == null) {
                        if (features.contains(Feature.VISIT_NULL)) visitor.visit(chain.appendField(field, null));
                        continue;
                    }
                    boolean isPrimitive = field.getType().isPrimitive();
                    Chain extendedChain = chain.appendField(field, childValue);
                    if (isPrimitive) {
                        if (features.contains(Feature.VISIT_PRIMITIVES)) visitor.visit(extendedChain);
                    } else {
    ...
    public static final Predicate<Chain> notEnumFieldsOrClasses = chain -> !(Enum.class.isAssignableFrom(chain.getValueType()) || chain.getValue() instanceof Class<?>);

    public static final Predicate<Chain> skipTransientFields = chain -> !(chain instanceof FieldChain) || (((FieldChain) chain).getField().getModifiers() & Modifier.TRANSIENT) == 0;

    public static final Function<Chain, Object> chainToObject = Chain::getValue;
